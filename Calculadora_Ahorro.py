...
#  by Erick Perez  |  v1.0 RC — Modern Fluent UI
...
def show_splash(root, on_done):
    ...
    cv.create_text(CX, cy0+132, text="by Erick Perez  •  v1.0 RC Fluent Edition",
...
class CalcApp(tk.Frame):
    ...
    def _step_hogar(self):
        ...
        g.pack(fill="x", padx=16, pady=(0,16))
        g.columnconfigure(0,weight=1); g.columnconfigure(1,weight=1)
        for i,(k,lk) in enumerate([("alquiler","p3_rent"),("internet","p3_net"),
                                    ("luz","p3_elec"),("agua","p3_water"),("data_movil","p3_mobile")]):
            w = FluentEntry(g, label=T(lk), prefix=sym)
